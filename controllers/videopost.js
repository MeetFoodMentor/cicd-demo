...
const filterVideoPost = async (videoPosts, userSub) => {
  if (!userSub) {
    return videoPosts;
  }

  const videoReports = await VideoReport.find({ userSub });
  let reportedVideoIds = new Set();
  for (let videoReport of videoReports) {
    reportedVideoIds.add(String(videoReport.videoPost));
  }
  const user = await User.findOne({ userId: userSub });

  return videoPosts.filter((videoPost) => {
    if (reportedVideoIds.has(String(videoPost._id))) {
      return false;
    }
    let isBlocked = false;
    try {
      isBlocked = user.blockedUsers
        .map((user) => user.user)
        .includes(videoPost.userId._id);
    } catch (e) {
      isBlocked = user.blockedUsers
        .map((user) => user.user)
        .includes(videoPost.userId);
    }
    return !isBlocked;
  });
};
...
exports.getVideoPost = async (req, res) => {
  ...
  try {
    ...
    res.json(await filterVideoPost(videoPosts, userSub));
  } catch (err) {
...
exports.fetchVideoPost = async (req, res) => {
  ...
  try {
    ...
    res.json(await filterVideoPost(videoPosts, req.userSub));
  } catch (err) {
    return res.status(500).json({
      errors: [
        {
